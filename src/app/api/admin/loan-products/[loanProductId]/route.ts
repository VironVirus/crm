...
export async function PATCH(
  ...
  try {
    ...
    return jsonError("Unable to read the loan product update.", 400);
  }
  ...
  if (!user) {
    return jsonError("You need to sign in before updating a loan product.", 401);
  }
  ...
  if (profile?.role !== "admin") {
    return jsonError("Only administrators can update loan products.", 403);
  }
